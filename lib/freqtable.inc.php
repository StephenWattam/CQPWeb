...
function corpus_make_freqtables()
{
	...
	$sql_query = "load data local infile '$filename' into table $temp_tablename fields escaped by ''";
	$result = mysql_query($sql_query, $mysql_link);
	if ($result == false)
		exiterror_mysqlquery(mysql_errno($mysql_link),
			mysql_error($mysql_link), __FILE__, __LINE__);
...
// to restrictions in all cases where this function is called or not !!
	...
	$sql_query = "LOAD DATA LOCAL INFILE '$temp_table_loadfile' into table $temp_table fields escaped by ''";
	$result = mysql_query($sql_query, $mysql_link);
	if ($result == false) 
		exiterror_mysqlquery(mysql_errno($mysql_link), 
			mysql_error($mysql_link), __FILE__, __LINE__);
