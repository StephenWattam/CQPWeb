...
if ( isset($_GET['kwWhatToShow']) && in_array($_GET['kwWhatToShow'], array('allKey', 'onlyPos', 'onlyNeg', 'lock')))
	...
	exiterror_general("The system could not detect whether you selected positive keywords, negative keywords, or lockwords.");
...
if (!isset($_GET['kwStatistic']) )
	exiterror_general("No statistic was specified!");
if (! array_key_exists($_GET['kwStatistic'], $stat_sort_col_head))
	exiterror_general("An invalid statistic was specified!");
...
if ($mode == 'lock')
{
	...
	{
	case 'LL':
		exiterror_general('Lockword calculations cannot be performed with the log-likelihood statistic. '
			. 'Try Log Ratio (unfiltered or with Confidence Interval filter) instead.');
	case 'LR_LL':
		exiterror_general('Lockword calculations cannot be performed with a log-likelihood filter. '
			. 'Use unfiltered Log Ratio, or Log Ratio with Confidence Interval filter, instead.');
	}
}

// TODO use JavaScript to disable selection of this pair of options in the initial page.
...
if ($alpha >= 1.0 && $statistic == 'LR_CI')
	exiterror_general('You asked for Log Ratio with a Confidence Interval filter, but you didf not specify the size of Confidence Interval...');
...
if (isset($_GET['kwTable1']) )
	...
	exiterror_general("No frequency list was specified (table 1)!");
...
if (isset($_GET['kwTable2']) )
	...
	exiterror_general("No frequency list was specified (table 2)!");
...
/* get a string to put into linked queries with the subcorpus */
...
if ($table_foreign[2])
	if (1 < mysql_num_rows(do_mysql_query("show tables like '{$table_name[2]}'")))
		exiterror_general("The corpus or subcorpus you selected for list (2) does not seem to have tags called &ldquo;$att_for_com&rdquo;");
...
foreach (array(1, 2) as $i)
{
	...
	if (NULL === $corpus_tokens[$i])
		exiterror_general("sum(freq) not found in from {$table_name[$i]}, null value returned from MySQL.");		
...
if ($statistic != 'LR_CI')
{
	 ...
	 * we don't get an incorrect answer.
	 *
	 * number of sig figs is based on the default in R. 
	 */
...
default:
	...
	exiterror_general('Assert: bad logic failure', __FILE__, __LINE__);
	break;
}
switch ($what_to_show)
{
...
//show_var(mysql_fetch_object($result));exit;
	for ( $i = 0 ; $i < $n ; $i++ )
		echo "\n\t<tr>"
...
function print_keyword_line_plaintext($data, $line_number, $da, $corpus_size)
{
...
function keywords_write_download($att_desc, $description, &$result, $corpus_size)
{
	...
	for ($i = 1; ($r = mysql_fetch_object($result)) !== false ; $i++ )
		echo print_keyword_line_plaintext($r, $i, $da, $corpus_size);
}
